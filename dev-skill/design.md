...
> **全局红线**：谋定后动,设计未经确认不得开始编码；删代码确认；按需执行,不主动多做未要求的事。
> **本阶段红线**：设计第一步是技术选型；必须输出模块拆分/修改逻辑/技术路线；只有用户明确"同意"后才能进入实现阶段。
...
- [ ] 技术选型已完成（全新开发有选型结论,修改项目确认是否沿用）
- [ ] 模块拆分方案已输出
- [ ] 修改逻辑已输出（伪代码或步骤）
- [ ] 技术路线分歧已列出优缺点+推荐（如有）
- [ ] 横切规则已检查（运行安全/LLM 集成,如涉及）
- [ ] 用户已确认"设计方案同意"

全部勾选后,本阶段结束,进入实现阶段（遵循 [代码编写规范](coding-standards.md)）。设计文档同步更新。
