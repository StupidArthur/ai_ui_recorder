...
> **全局红线**：谋定后动；删代码确认；按需执行,验证类操作前先征求同意。
> **本阶段红线**：核心业务逻辑必须有单元测试；测试不污染真实环境（用临时目录隔离）；测试之间相互独立。
...
- [ ] 核心业务逻辑已有单元测试（不依赖框架 runtime 的纯逻辑）
- [ ] 四类场景已覆盖（正常路径/空输入/错误输入/边界情况）
- [ ] 测试用临时目录隔离,不污染真实环境
- [ ] 测试之间相互独立,不依赖执行顺序
- [ ] 测试记录已输出（写法见 [文档与知识管理](doc-management.md)）

全部勾选后,本阶段结束,进入 [版本管理](version-control.md)。
