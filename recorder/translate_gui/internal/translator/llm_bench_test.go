...
func TestM3ThinkingBench(t *testing.T) {
	...
	if errDisabled != nil {
		...
		t.Logf("content 预览:\n%s", truncateBenchmarkLog(resDisabled.Content, 1000))
	}
	...
	if errAdaptive != nil {
		...
		t.Logf("content 预览:\n%s", truncateBenchmarkLog(resAdaptive.Content, 1000))
	}
...
func TestM3DiffImprovedVerify(t *testing.T) {
	...
	if err != nil {
		...
		t.Logf("content 预览:\n%s", truncateBenchmarkLog(res.Content, 1500))
...
func truncateBenchmarkLog(value string, limit int) string {
	if len(value) <= limit {
		return value
	}
	return value[:limit] + "..."
