...
### 4.8 试用模型中转工具（Go，独立部署）

- **能力**：提供独立 Go 中转服务，兼容 `POST /v1/chat/completions`，主工程可只改配置接入。
  - **验收**：
    - `tool/ai-proxy-go` 可独立启动，不依赖主工程进程
    - 主工程 `config/ai.local.json` 指向中转后可正常翻译

- **能力**：提供内嵌流式问答验证页，快速验证中转与模型可用性。
  - **验收**：
    - 访问中转根路径 `/` 可打开验证页
    - 输入 trial key + 问题后可看到流式输出

- **能力**：支持中转统一管控（总开关、trial key、基础限流）。
  - **验收**：
    - `enabled=false` 时请求被统一拒绝
    - 非法 trial key 返回鉴权失败
    - 超出频率上限返回限流错误
