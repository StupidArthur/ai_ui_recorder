...
- **Version**: 1.0
- **Language**: 中文
- **Description**: 将 Phase 1 结构化步骤按**业务逻辑**聚合为供下游无视觉 Agent 执行的逻辑步骤 JSON，由程序渲染为 case_4_agents.txt。

## Background
下游 Agent **无视觉**，仅依赖文本中的 DOM 特征（文案、placeholder、输入值）定位元素。Phase 1 步骤过细且偏物理动作，需聚合为 logicalName + microActions。本 Skill 运行于 **Phase 4**，滑动窗口处理长流程。
