...
<steps>
  <step id="1">
    ...
    <assertText>用户名输入框中显示已输入的手机号</assertText>
  </step>
  <step id="2">
    <action>点击可见文本为「立即登录」的按钮</action>
    <observation>页面发生跳转，主区域出现工作台内容</observation>
    <assertText>页面跳转至工作台，地址栏 URL 变化</assertText>
  </step>
</steps>
```
...
| `assertText` | 是 | 该步骤的客观完成标准：操作后可在 DOM/页面上验证的可见标志（如某元素出现/消失、某文本显示、某状态切换）。基于 observation 中的实际变化提炼，禁止主观臆测业务结果。无可见变化时写「无可见变化」 |
